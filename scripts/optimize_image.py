...
def _best_jpeg_quality(img: Image.Image) -> tuple[bytes, int]:
    ...
    A fixed quality percentage produces wildly different file sizes across
    photos of different resolution and detail, so it's the size we actually
    care about (what the large-files hook checks), not the quality number,
...
def optimize_image(src: Path, dest: Path) -> None:
    ...
    with Image.open(src) as img:
        ...
        # Fix phone/camera image orientation using EXIF metadata.
        # Some photos are stored sideways and rely on EXIF Orientation
        # for image viewers to display them correctly.
...
def main() -> int:
    ...
    if args.input.is_dir():
        ...
        if not found:
            print(
                f"No images found under {args.input}",
                file=sys.stderr,
            )
            return 1
