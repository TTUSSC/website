...
<template>
  <header
    ...
    :class="isScrolled
      ? 'bg-paper/90 backdrop-blur-xl shadow-[0_1px_0_var(--color-chalk)]'
      : 'bg-paper'"
  >
    <nav class="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
      ...
      <div class="hidden md:flex items-center gap-2">
        <a href="https://discord.com/invite/29PsKfe45h" target="_blank"
          class="px-3.5 py-1.5 text-[13px] font-medium text-clay border border-chalk rounded-md hover:border-ink hover:text-ink transition-all duration-200">
          Discord
        </a>
        <a href="https://buymeacoffee.com/ttussc" target="_blank"
      ...
      <!-- Mobile -->
      <button @click="toggleMenu" class="md:hidden p-1.5 rounded-md hover:bg-fog transition-colors" aria-label="Menu">
      ...
      <div v-if="isMenuOpen" class="md:hidden border-t border-chalk bg-paper">
        <div class="px-6 py-3 space-y-0.5">
          <RouterLink v-for="link in navLinks" :key="link.to" :to="link.to"
            ...
            active-class="!text-ink !bg-fog" @click="closeMenu">{{ link.label }}</RouterLink>
          <div class="pt-3 flex gap-2">
            <a href="https://discord.com/invite/29PsKfe45h" target="_blank"
              class="flex-1 text-center px-3 py-2 text-sm border border-chalk rounded-md text-ink" @click="closeMenu">Discord</a>
            <a href="https://buymeacoffee.com/ttussc" target="_blank"
              class="flex-1 text-center px-3 py-2 text-sm bg-ink text-paper rounded-md" @click="closeMenu">贊助</a>
          </div>
        </div>
      </div>
    </transition>
